...
/* Mirrors Yoga200CombinedSchema exactly */
interface CombinedContent {
  ...
  /* CONTENT 2 fields — now in same schema */
  evalDesc: string;
  schedDesc: string;
...
/* ══════════════════════════════
   VINTAGE HEADING
══════════════════════════════ */
function VintageHeading({ children, para }: { children: React.ReactNode; para?: React.ReactNode }) {
  if (para) {
    return (
...
/* ══════════════════════════════
   VIDEO SECTION
══════════════════════════════ */
function VideoSection({ videoUrl, badgeText }: { videoUrl?: string; badgeText?: string }) {
  /* Support YouTube embed URL or fallback */
  const embedSrc = videoUrl
    ? videoUrl
    : "https://www.youtube.com/embed/v7AYKMP6rOE?autoplay=1&mute=1&loop=1&playlist=v7AYKMP6rOE&controls=0&showinfo=0&rel=0&modestbranding=1&iv_load_policy=3";
      ...
      <div className={styles.videoCard}>
        ...
        <div className={styles.videoBadge}>
          ...
          <span className={styles.videoBadgeTxt}>
            {badgeText || "Yoga Teacher Training · Rishikesh"}
          </span>
        </div>
      </div>
    </div>
...
/* ══════════════════════════════
   COURSE INFO CARD
   — reads from CombinedContent courseCard fields
══════════════════════════════ */
function CourseInfoCard({
  ...
  /* Build details from DB fields, fallback to hardcoded defaults */
  const details = [
    {
      icon: <DurationIcon />,
  ...
  /* Price display — prefer DB values over computed */
  const displayOldPrice = content?.courseCardOldPrice || `$${originalPrice}`;
  const displayNewPrice = content?.courseCardNewPrice || `$${startingPrice}`;
...
function PremiumSeatBooking({
  ...
  const fmtPriceAdvanced = (batch: Batch | null, overrideUsd?: number): { amount: string; cur: string } => {
    if (!batch && overrideUsd === undefined) return { amount: "—", cur: currency };

    if (currency === "INR") {
      if (batch?.inrFee) {
        const num = parseFloat(batch.inrFee.replace(/[₹,]/g, "").trim());
        if (!isNaN(num) && num > 100) return { amount: `₹${num.toLocaleString("en-IN")}`, cur: "INR" };
      }
      const usdNum = batch
    ...
    return (
      <div className={styles.currDrop} ref={ref}>
        <button className={styles.currDropBtn} onClick={() => setOpen((p) => !p)} aria-expanded={open} type="button">
          ...
          <span className={styles.currDropLabel}>{currency === "USD" ? "English" : "हिन्दी"}</span>
            ...
            {(["USD", "INR"] as Currency[]).map((c) => (
              ...
              >
                ...
                <div className={styles.currDropItemText}>
                  <span className={styles.currDropItemCode}>{c === "USD" ? "English" : "हिन्दी"}</span>
                  ...
                  >
                    ...
                    {!full && (
                      <>
                        ...
                        <span className={styles.psbBcSeatsBadge} style={{ color: low ? "#c8700a" : "#3d6000", borderColor: low ? "#c8700a" : "#3d6000" }}>
          ...
          <div className={styles.psbRpBody}>
            ...
            <div className={styles.psbPriceRow}>
              <div className={styles.psbPriceCard}>
                <div className={styles.psbPcAmt}>
                  {selected ? (currency === "INR" ? `₹${Math.round(selected.privatePrice * rate)}` : `$${selected.privatePrice}`) : "—"}
              ...
              <div className={styles.psbPriceCard}>
                <div className={styles.psbPcAmt}>
                  {selected ? (currency === "INR" ? `₹${Math.round(selected.twinPrice * rate)}` : `$${selected.twinPrice}`) : "—"}
                  <span className={styles.psbPcCur}>{currency}</span>
                </div>
            ...
            <div className={styles.psbPriceWide}>
              <div className={styles.psbPwLeft}>
                <span className={styles.psbPcAmt} style={{ fontSize: "1rem" }}>
                  {selected ? (currency === "INR" ? `₹${Math.round(selected.dormPrice * rate)}` : `$${selected.dormPrice}`) : "—"}
                </span>
                <span className={styles.psbPcCur}>{currency}</span>
              </div>
            ...
            {selected && currency === "USD" && (
              <div className={styles.psbInrRow}>
                ...
                <span className={styles.psbInrAmt}>{selected.usdFee.startsWith("$") ? selected.usdFee : `$${selected.usdFee}`}</span>
              </div>
            )}
...
export default function TwoHundredHourYoga() {
    ...
    const fetchAll = async () => {
      try {
        ...
        const [contentRes, batchRes] = await Promise.all([
          api.get("/yoga-200hr/content"),        // ← single API
          api.get("/200hr-seats/getAllBatches"),
        ]);

        /* API returns array → pick Active or first */
  ...
  /* ── Derived data from single content object ── */
    ...
    <div className={styles.root}>
      ...
      <section className={styles.heroSection2}>
        <VintageHeading
          para={
            content?.introPara1 ? (
              <span className={styles.bodyText}>{stripHtml(content.introPara1)}</span>
            ) : undefined
          }
        >
        ...
        {/* Video — uses DB videoUrl if available */}
        <VideoSection videoUrl={content?.videoUrl} badgeText={content?.videoBadgeText} />
        ...
        {(content?.aimsIntro?.length || content?.aimsBullets?.length) && (
          <div className={styles.h3LeftWrap}>
            <div className={styles.h3LeftContent}>
              <h3 className={styles.h3Left}>Aims & Objectives</h3>
              ...
              {content?.aimsOutro && (
                <p className={styles.bodyText}>{stripHtml(content.aimsOutro)}</p>
              )}
            </div>
      ...
      <section id="curriculum" className={styles.contentSection2}>
        <div style={{ marginTop: "2.5rem" }}>
          <h3 className={styles.h3Left}>Course Syllabus</h3>
          <div className={styles.underlineBar} />
            ...
            <div className={styles.moduleDetailGrid}>
              <div className={styles.moduleDetailImg}>
                {content.ashtangaImage && (
                  <img src={imgUrl(content.ashtangaImage)} alt="Ashtanga" className={styles.modImg} />
                )}
              </div>
              <div>
                <p className={styles.bodyText}>{stripHtml(content.ashtangaDesc)}</p>
              </div>
            </div>
          </>
        )}
          ...
          <div className={styles.section3Split}>
            <div className={styles.section3Left}>
              <div className={styles.primaryCurrCard}>
                ...
                <div className={styles.underlineBar} />
                <p className={styles.bodyText}>{stripHtml(content.primaryIntro)}</p>
        ...
        {content?.hathaDesc && (
          <>
            ...
            <div className={styles.moduleDetailGrid}>
              <div className={styles.moduleDetailImg}>
                {content.hathaImage && (
                  <img src={imgUrl(content.hathaImage)} alt="Hatha" className={styles.modImg} />
                )}
              </div>
              <div>
                <p className={styles.bodyText}>{stripHtml(content.hathaDesc)}</p>
              </div>
            </div>
          </>
        )}
          ...
          <div className={styles.asanaFilterRow}>
            {ASANA_FILTERS.map((f) => (
              ...
              >
                {f}
                {f !== "All Poses" && (
                  <span style={{ marginLeft: 4, opacity: 0.7, fontSize: "0.75em" }}>
      ...
      <section id="facility" className={styles.contentSection}>
            ...
            <VintageHeading
              para={
                <span className={styles.bodyText}>
                  Experience world-class facilities designed to support your yoga journey with comfort and serenity.
                </span>
              }
            >
              Luxury Facilities
            ...
            <div className={styles.luxuryLayout}>
              ...
              <div className={styles.luxuryCenterImg}>
                {content.luxImages?.length ? (
                  <img src={imgUrl(content.luxImages[0])} alt="Luxury facility" className={styles.luxCenterImgEl} />
                ) : (
                  <img src="https://images.unsplash.com/photo-1603988363607-e1e4a66962c6?w=500&h=600&fit=crop" alt="Luxury facility" className={styles.luxCenterImgEl} />
                )}
              </div>
            ...
            {content.luxImages?.length > 1 && (
              <div className={styles.luxuryExtraImgs}>
                {content.luxImages.slice(1).map((src, i) => (
                  <img key={i} src={imgUrl(src)} alt={`Facility ${i + 2}`} className={styles.luxuryExtraImg} />
                ))}
              </div>
            )}
          </>
        ...
        {content?.indianFees?.length ? (
          <>
            <VintageHeading>Indian Student Fee</VintageHeading>
            <div className={styles.indianFeeGrid}>
              {content.indianFees.map((f, i) => (
                <div key={i} className={styles.indianFeeCard}>
                  ...
                  <a href="#dates-fees" className={styles.indianFeeBtn}>Book Now</a>
                </div>
              ))}
            </div>
          </>
        ...
        {content?.schedRows?.length ? (
          <>
            <VintageHeading
              para={
                content.schedDesc ? (
                  <span className={styles.bodyText}>{stripHtml(content.schedDesc)}</span>
                ) : undefined
              }
            >
              Daily Schedule
            ...
            <div className={styles.schedLayout}>
                ...
                <table className={styles.schedTable}>
                  ...
                  <tbody>
                    {content.schedRows.map((r, i) => (
                      <tr key={i}>
                        <td className={styles.schedTime}>{stripHtml(r.time)}</td>
              ...
              <div className={styles.schedImgGrid}>
                {content.schedImages?.length ? (
                  <img src={imgUrl(content.schedImages[0])} alt="Yoga class schedule" className={styles.schedImg} />
                ) : (
      ...
      <section className={styles.contentSection}>
          ...
          <div className={styles.infoBlock}>
            ...
            <ol className={styles.numberedListSimple}>
              {content.instrLangs.map((l, i) => (
                <li key={i}>{typeof l === "string" ? stripHtml(l) : stripHtml((l as any).lang)}</li>
              ))}
            </ol>
          </div>
        ) : null}
        ...
        {content?.visaPassportDesc && (
          <div className={styles.infoBlock}>
            ...
            <p className={styles.bodyText}>{stripHtml(content.visaPassportDesc)}</p>
          </div>
        )}
        ...
        <div className={styles.ctaBanner}>
          ...
          <div className={styles.ctaBannerRight}>
            ...
            <a href="/yoga-registration?type=200hr" className={styles.applyNowBtn}>Apply Now</a>
            <a
            ...
            {content.programs.map((p, i) => (
              <div key={i} className={styles.programCard}>
                <div className={styles.programCardImg}>
                  <img
                    src={p.image ? imgUrl(p.image) : programImages[i % programImages.length]}
                    alt={stripHtml(p.title)}
                  />
                  <div className={styles.programImgOverlay} />
                  {i === 0 && <span className={styles.programBadge}>POPULAR</span>}
                </div>
      ...
      {content?.knowQA?.length ? (
        <section className={styles.contentSection}>
          <VintageHeading>Requirements & Information</VintageHeading>
          <div className={styles.requirementsTextFull}>
            {content.knowQA.slice(0, 2).map((item, i) => (
            ...
            <div className={styles.videoSectionWrap}>
              ...
              <div className={styles.reqVideoBox}>
                <iframe
                  src={
                    content?.videoUrl ||
                    "https://www.youtube.com/embed/v7AYKMP6rOE?autoplay=1&mute=1&loop=1&playlist=v7AYKMP6rOE&controls=0&modestbranding=1&rel=0&showinfo=0"
                  }
            ...
            {content.knowQA.slice(2).map((item, i) => (
